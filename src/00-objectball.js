...
function playerNumbers(teamName) {
    if (teamName === homeTeam().teamName) {
        const homeNumbers = Object.values(homeTeam().players).map(function (homePlayer) {
            return [homePlayer.number]
        })
        return homeNumbers.flat()
    } 
    else (teamName === awayTeam().teamName) 
    const awayNumbers = Object.values(awayTeam().players).map(function (awayPlayer) {
...
function bigshoeRebounds() {
    let shoesArr = Object.values(players()).map(el => el.shoe)
    let biggestShoe = Math.max(...shoesArr)
    for (let player in players()) {
        if (biggestShoe === players()[player].shoe) {
